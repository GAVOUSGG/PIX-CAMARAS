...
const Logistics = ({
  ...
  const filteredShipments = useMemo(() => {
    return shipmentsData.filter((shipment) => {
      // Filtro por bÒºsqueda en ID, destino, destinatario o tracking
      const matchesSearch =
        searchTerm === "" ||
  ...
  const handleSaveShipment = async (shipmentData) => {
    try {
      if (editingShipment) {
        ...
        alert("EnvÒ­o actualizado correctamente");
      } else {
        await onCreateShipment(shipmentData);
        alert("EnvÒ­o creado correctamente");
      }
      setShowForm(false);
      setEditingShipment(null);
    } catch (error) {
      alert("Error al guardar el envÒ­o");
    }
  };
  ...
  const handleDeleteShipment = async (shipmentId) => {
    try {
      ...
      alert("EnvÒ­o eliminado correctamente");
    } catch (error) {
      alert("Error al eliminar el envÒ­o");
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className={`text-2xl md:text-3xl font-bold tracking-tight transition-colors duration-500 ${darkMode ? 'text-white' : 'text-zinc-900'}`}>
            LogÒ­stica
          </h2>
          <p className="text-zinc-500 text-sm mt-1">
            <span className="text-emerald-400 font-bold">{filteredShipments.length}</span> de {shipmentsData.length} envÒ­os gestionados
          </p>
        </div>
        <button
      ...
      <div className={`rounded-2xl border p-6 transition-all duration-500 ${
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-2.5 ml-1 leading-none">Estado del envÒ­o</label>
            <select
              value={statusFilter}
          ...
          <div>
            <label className="block text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-2.5 ml-1 leading-none">Destino / Sede</label>
            <select
            ...
            >
              <option value="todos" className={darkMode ? "bg-zinc-900" : "bg-white"}>Todos los destinos</option>
              {uniqueDestinations.map((destination) => (
                <option key={destination} value={destination} className={darkMode ? "bg-zinc-900" : "bg-white"}>{destination}</option>
              ))}
            </select>
          </div>
        </div>
      ...
      {filteredShipments.length === 0 && (
        <div className={`text-center py-20 rounded-3xl border border-dashed transition-all duration-500 ${
          ...
          <div className={`text-lg mb-4 font-medium transition-colors duration-500 ${darkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>
            {hasActiveFilters
              ? "No se encontraron envÒ­os con los filtros aplicados"
              : "No hay envÒ­os registrados"}
          </div>
          {hasActiveFilters && (
            <button
              onClick={clearFilters}
