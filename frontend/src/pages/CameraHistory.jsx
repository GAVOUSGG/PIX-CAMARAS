...
const CameraHistory = ({ darkMode = true }) => {
  ...
  if (loading) {
    return (
      <div className={`flex items-center justify-center p-12 transition-colors duration-500 ${darkMode ? 'bg-zinc-900' : 'bg-zinc-50'}`}>
        <div className={`text-xl flex items-center space-x-3 ${darkMode ? 'text-white' : 'text-zinc-900'}`}>
    ...
    <div className={`space-y-6 animate-fade-in`}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className={`text-2xl md:text-3xl font-bold tracking-tight transition-colors duration-500 ${darkMode ? 'text-white' : 'text-zinc-900'}`}>
      ...
      <div className={`rounded-2xl border p-6 transition-all duration-500 ${
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          >
            <option className={darkMode ? "bg-zinc-900" : "bg-white"} value="all">Todas las cámaras</option>
            {cameras.map((camera) => (
              <option className={darkMode ? "bg-zinc-900" : "bg-white"} key={camera.id} value={camera.id}>
      ...
      {filterCamera === "all" ? (
        <div className={` border transition-all duration-500 overflow-hidden relative min-h-[600px] flex items-center justify-center ${
          darkMode ? 'bg-zinc-900/30 border-white/5 shadow-2xl' : 'bg-white border-black/5 shadow-xl shadow-zinc-200'
        }`}>
          <div className="text-center p-8 max-w-lg mx-auto">
            
            <h2 className={`text-4xl font-black tracking-tight mb-4 ${darkMode ? 'text-white' : 'text-zinc-900'}`}>
              Selector
            </h2>
            <p className={`text-lg font-medium tracking-tight mb-8 ${darkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>
