    ...
    <tr className={`transition-colors duration-300 ${darkMode ? 'hover:bg-white/[0.02]' : 'hover:bg-zinc-50'}`}>
      <td className="px-6 py-5 whitespace-nowrap">
        <div className="flex flex-col">
          <div className={`text-sm font-black transition-colors duration-500 ${darkMode ? 'text-white' : 'text-zinc-900'}`}>{worker.name}</div>
          <div className="flex items-center gap-2 mt-0.5">
            <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-tight">ID: {worker.id}</span>
            {worker.specialty && (
              <>
      ...
      <td className="px-6 py-5 whitespace-nowrap">
        <div className={`text-xs font-bold transition-colors duration-500 ${darkMode ? 'text-zinc-300' : 'text-zinc-700'}`}>{worker.state}</div>
      </td>
      <td className="px-6 py-5 whitespace-nowrap">
        <div className={`text-xs font-mono font-bold transition-colors duration-500 ${darkMode ? 'text-zinc-300' : 'text-zinc-700'}`}>{worker.phone}</div>
      </td>
      <td className="px-6 py-5 whitespace-nowrap">
      ...
      <div className="md:hidden space-y-4 p-4">
        {workers && workers.length > 0 ? (
          ...
          <div className={`text-center py-10 rounded-2xl border border-dashed ${darkMode ? 'bg-white/5 border-white/10' : 'bg-zinc-50 border-zinc-200'}`}>
            <p className="text-xs font-black text-zinc-500 uppercase tracking-widest">Sin operadores registrados</p>
          </div>
        )}
      </div>
          ...
          <thead className={darkMode ? 'bg-white/[0.02]' : 'bg-zinc-50/50'}>
            <tr>
              {[
                ...
                <th key={h} className={`px-6 py-4 text-left text-[10px] font-black uppercase tracking-[0.2em] ${darkMode ? 'text-zinc-500' : 'text-zinc-400'} ${i === 4 ? 'text-right' : ''}`}>
                  {h}
                </th>
              ))}
            </tr>
            ...
            {workers && workers.length > 0 ? (
              ...
              <tr>
                <td colSpan="5" className="px-6 py-20 text-center">
                  <p className="text-[10px] font-black uppercase tracking-widest md:tracking-[0.4em] text-zinc-500 opacity-20">Base de datos vacía</p>
                </td>
              </tr>
            )}
          </tbody>
