    ...
    if (camera.assignedTo) {
      return (
        <div>
          ...
          {camera.assignedWorkerId && <div className="text-[10px] font-bold text-zinc-500">ID: {camera.assignedWorkerId}</div>}
        </div>
      );
    }
    return (
      <div className="opacity-60">
        <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500 italic">No asignada</span>
      </div>
    );
  };

  return (
    <tr className={`transition-all duration-300 ${darkMode ? 'hover:bg-white/[0.02]' : 'hover:bg-zinc-50'}`}>
      <td className="px-6 py-5 whitespace-nowrap">
        <div>
          <div className={`text-sm font-black font-mono transition-colors duration-500 ${darkMode ? 'text-white' : 'text-zinc-900'}`}>{camera.id}</div>
      ...
      <td className="px-6 py-5 whitespace-nowrap">
        <div className="flex items-center space-x-2">
          ...
          <div className={`text-xs font-mono font-black tracking-tighter transition-colors duration-500 ${darkMode ? 'text-zinc-300' : 'text-zinc-600'}`}>{camera.serialNumber}</div>
        </div>
      </td>
      <td className="px-6 py-5 whitespace-nowrap">
        <div className={`text-xs font-mono font-black tracking-tighter transition-colors duration-500 ${darkMode ? 'text-zinc-300' : 'text-zinc-600'}`}>{camera.simNumber || <span className="opacity-40 italic">N/A</span>}</div>
      </td>
      <td className="px-6 py-5 whitespace-nowrap">
        <StatusBadge status={camera.status} darkMode={darkMode} />
      </td>
      <td className="px-6 py-5 whitespace-nowrap">
        <div className={`text-xs font-bold transition-colors duration-500 ${darkMode ? 'text-zinc-300' : 'text-zinc-700'}`}>{camera.location}</div>
      </td>
      <td className="px-6 py-5 whitespace-nowrap">
        ...
        <div className="flex items-center space-x-2 justify-end">
          ...
          <div className="relative">
            <button
              ...
              className={`p-2.5 rounded-xl transition-all duration-300 ${
                darkMode ? 'bg-white/5 text-zinc-400 hover:bg-white/10 hover:text-white' : 'bg-zinc-100 text-zinc-500 hover:bg-zinc-200'
              }`}
            >
            ...
            {showMenu && (
              <>
                ...
                <div className={`absolute right-0 mt-3 w-56 rounded-[1.5rem] border shadow-2xl z-50 overflow-hidden transform origin-top-right transition-all duration-300 ${
                  darkMode ? 'bg-zinc-900 border-white/10 shadow-black/50' : 'bg-white border-black/5 shadow-xl'
                }`}>
    ...
    <>
      <div className="md:hidden space-y-4 p-4">
        {cameras && cameras.length > 0 ? (
          ...
          <div className={`text-center py-10 rounded-2xl border border-dashed transition-all duration-500 ${darkMode ? 'bg-white/5 border-white/10' : 'bg-zinc-50 border-zinc-200'}`}>
            <p className="text-xs font-black text-zinc-500 uppercase tracking-widest">Sin registros de cámaras</p>
          </div>
        )}
      </div>
          ...
          <thead className={darkMode ? 'bg-white/[0.02]' : 'bg-zinc-50/50'}>
            <tr>
              {[
                ...
                <th key={h} className={`px-6 py-4 text-left text-[10px] font-black uppercase tracking-[0.2em] ${darkMode ? 'text-zinc-500' : 'text-zinc-400'} ${i === 6 ? 'text-right' : ''}`}>
                  {h}
                </th>
              ))}
            </tr>
              ...
              <tr>
                <td colSpan="7" className="px-6 py-20 text-center">
                  <div className="flex flex-col items-center gap-4 opacity-20">
                    <Camera className={`w-12 h-12 ${darkMode ? 'text-white' : 'text-zinc-900'}`} />
                    <p className="text-[10px] font-black uppercase tracking-widest md:tracking-[0.4em] text-zinc-500">Base de datos de unidades vacía</p>
                  </div>
                </td>
              </tr>
            )}
