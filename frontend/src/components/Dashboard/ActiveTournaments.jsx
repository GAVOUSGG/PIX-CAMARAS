      ...
      <div className={`rounded-3xl p-5 lg:p-6 h-full border shadow-lg relative overflow-hidden transition-all duration-500 transform-gpu ${
        ...
        <div className="flex items-center justify-between mb-6 relative z-10">
          <div className="flex items-center gap-2.5">
             ...
             <h3 className={`text-lg font-bold tracking-tight transition-colors duration-500 ${darkMode ? 'text-white' : 'text-zinc-900'}`}>Torneos Activos</h3>
          </div>
        ...
        <div className="space-y-4 relative z-10">
          {activeTournaments.length > 0 ? (
              ...
              <div 
                ...
                className={`group/item border p-4 lg:p-5 rounded-2xl transition-all duration-300 cursor-pointer relative overflow-hidden shadow-sm ${
                  darkMode 
                    ...
                    : 'bg-zinc-50 border-black/5 hover:border-emerald-500/20 hover:bg-white'
                }`}
                onClick={() => setSelectedTournament(tournament)}
              >
                <div className="flex items-center justify-between relative z-10">
                  <div className="flex items-center space-x-4">
                    <div className={`p-2 rounded-xl border hidden sm:block shadow-inner transition-colors duration-500 ${darkMode ? 'bg-zinc-950 border-white/5' : 'bg-white border-black/5'}`}>
                      <StatusBadge status={tournament.status} />
                    </div>
                    <div>
                      <h4 className={`font-bold text-base lg:text-lg mb-0.5 tracking-tight transition-colors duration-300 ${
                        darkMode ? 'text-white group-hover/item:text-emerald-300' : 'text-zinc-800 group-hover/item:text-emerald-600'
                      }`}>
                        {tournament.name}
                      </h4>
                      <p className={`text-[10px] lg:text-xs flex items-center gap-1.5 uppercase font-bold tracking-widest transition-colors duration-500 ${darkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>
                  ...
                  <div className="flex items-center gap-5">
                    <div className="text-right hidden sm:block">
                      <p className={`text-sm font-bold transition-colors duration-500 ${darkMode ? 'text-zinc-200' : 'text-zinc-700'}`}>{tournament.worker}</p>
                      <p className="text-zinc-500 text-[10px] uppercase tracking-widest font-black mt-0.5">
                        {tournament.cameras?.length || 0} CÒ¡maras
                      </p>
                    </div>
                    ...
                    <button 
                      className={`p-2.5 rounded-xl transition-all duration-300 border shadow-lg shrink-0 ${
                        darkMode 
                          ? 'bg-white/5 text-zinc-400 group-hover/item:text-emerald-400 group-hover/item:bg-emerald-500/10 border-transparent group-hover/item:border-emerald-500/20' 
                          : 'bg-white text-zinc-500 group-hover/item:text-emerald-600 group-hover/item:bg-emerald-50 border-black/5 group-hover/item:border-emerald-200'
                      }`}
                      title="Ver Detalles"
                    >
            ...
            <div className={`flex flex-col items-center justify-center py-16 text-center opacity-60 rounded-2xl border border-dashed transition-all duration-500 ${
              ...
              <p className="text-zinc-400 text-sm font-bold tracking-wide">No hay torneos activos</p>
            </div>
          )}
        </div>
      </div>
